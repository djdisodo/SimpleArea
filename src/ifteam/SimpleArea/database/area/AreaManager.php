...
class AreaManager
{
	...
	public function move(Player $player, $target) {
		if (is_numeric($target)) {
			$area = $this->areaProvider->getAreaToId($player->getLevel(), $target);
			...
			$properties = $this->properties->getUserProperties($target, $player->getLevel());
	...
	public function getList(Player $player) {
		$properties = $this->properties->getUserProperties($player->getName(), $player->getLevel());
	...
	public function autoCreate(Player $player) {
		...
		$areaHoldLimit = $this->whiteWorldProvider->get($player->getLevel())->getAreaHoldLimit();
		$userHoldCount = count($this->properties->getUserProperties($player->getName(), $player->getLevel()));
		if (!$player->isOp()) {
		...
		$defaultSize = $this->whiteWorldProvider->get($player->getLevel())->getDefaultAreaSize();
		$defaultXSize = ( int )round($defaultSize [0] / 2);
		$defaultZSize = ( int )round($defaultSize [1] / 2);

		$startX = ( int )round($player->x - $defaultXSize);
		$endX = ( int )round($player->x + $defaultXSize);
		$startZ = ( int )round($player->z - $defaultZSize);
		$endZ = ( int )round($player->z + $defaultZSize);

		$areaPrice = $this->whiteWorldProvider->get($player->getLevel())->getDefaultAreaPrice();

		$getOverlap = $this->areaProvider->checkOverlap($player->getLevel(), $startX, $endX, $startZ, $endZ);
		...
		$this->areaProvider->addArea($player->getLevel(), $startX, $endX, $startZ, $endZ, $player->getName(), true);
	...
	public function manualCreate(Player $player, $startX, $endX, $startZ, $endZ, $isHome = true) {
		...
		$this->areaProvider->addArea($player->getLevel(), $startX, $endX, $startZ, $endZ, $player->getName(), $isHome);
		$this->message($player, $this->get("areaset-success"));
		$this->message($player, $this->get("you-can-use-area-rent-command"));
	...
	public function give(Player $player, $target) {
		...
		if ($target instanceof Player) {
			$areaHoldLimit = $this->whiteWorldProvider->get($target->getLevel())->getAreaHoldLimit();
			$userHoldCount = count($this->properties->getUserProperties($target->getName(), $target->getLevel()));
			if ($userHoldCount >= $areaHoldLimit) {
	...
	public function saleList(Player $player, $index = 1) {
		$oncePrint = 20;
		$target = $this->properties->getSaleList($player->getLevel());
	...
	public function areaSizeUp(Player $player, $level, $id, $startX, $endX, $startZ, $endZ, $price) {
		$whiteWorld = $this->whiteWorldProvider->get($player->getLevel());
		if (!$player->isOp()) {
		...
		$area = $this->areaProvider->getAreaToId($level, $id);
		if (!$area instanceof AreaSection) {
			$this->alert($player, $this->get("area-doesent-exist"));
			$this->alert($player, $this->get("commands-area-info-help"));
			return false;
		...
		$bool = $this->areaProvider->resizeArea($level, $id, $startX, $endX, $startZ, $endZ, $resetFence);
		if ($bool) {
	...
	public function areaSizeDown(Player $player, $level, $id, $startX, $endX, $startZ, $endZ) {
		$whiteWorld = $this->whiteWorldProvider->get($player->getLevel());
		if (!$player->isOp()) {
			if (!$whiteWorld->isAllowAreaSizeUp()) {
	...
	public function initialPrivateArea(Player $player, Block $pos, $xSize, $zSize, $defaultY, $dmg) {
		...
		$getOverlap = $this->areaProvider->checkOverlap($pos->getLevel(), $startX, $endX, $startZ, $endZ);
		if ($getOverlap instanceof AreaSection) {
			$this->alert($player, $getOverlap->getId() . $this->get("automatic-post-fail-overlap-problem"));
			return false;
		}
		...
		$fenceBlock = Block::get($fenceId, $fenceDamange);
		$grassBlock = Block::get(Block::GRASS);

		$level = $pos->getLevel();
		$level->setBlock($pos->getSide(Vector3::SIDE_DOWN), $fenceBlock);

		$blockPos = new Vector3 ($pos->x, $defaultY, $pos->z);

		for ($x = $startX; $x <= $endX; $x++)
			for ($z = $startZ; $z <= $endZ; $z++)
				$level->setBlock($blockPos->setComponents($x, $defaultY - 1, $z), $grassBlock);

		$this->setSideFence($level, $defaultY, $startX, $startX, $startZ, $endZ, 2, $fenceBlock); // UP
		$this->setSideFence($level, $defaultY, $endX, $endX, $startZ, $endZ, 2, $fenceBlock); // DOWN
		$this->setSideFence($level, $defaultY, $startX, $endX, $startZ, $startZ, 2, $fenceBlock); // WEST
		$this->setSideFence($level, $defaultY, $startX, $endX, $endZ, $endZ, 2, $fenceBlock); // EAST
	...
	public function destructPrivateArea(Player $player, Block $pos, $xSize, $zSize, $defaultY, $dmg) {
		...
		$airBlock = Block::get(Block::AIR);

		$level = $pos->getLevel();
		$level->setBlock($pos->getSide(Vector3::SIDE_DOWN), $airBlock);
		...
		$this->setSideFence($level, $defaultY, $startX, $endX, $startZ, $startZ, 2, $airBlock); // WEST
		$this->setSideFence($level, $defaultY, $startX, $endX, $endZ, $endZ, 2, $airBlock); // EAST
		$this->message($player, $this->get("easy-automatic-post-complete"));
		return true;
	}

	public function setSideFence(Level $level, $defaultY, $startX, $endX, $startZ, $endZ, $length, Block $fenceBlock) {
		if ($startX > $endX) {
			$backup = $endX;
			$endX = $startX;
			$startX = $backup;
		...
		for ($x = $startX; $x <= $endX; $x++)
			for ($z = $startZ; $z <= $endZ; $z++)
				if ($fenceQueue <= $length) {
					$fenceQueue++;
					$level->setBlock($blockPos->setComponents($x, $defaultY, $z), $fenceBlock);
				} else {
